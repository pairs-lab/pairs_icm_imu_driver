...
namespace pairs_icm_imu_driver
{

/* class PairsIcmImuDriver //{ */

class PairsIcmImuDriver : public nodelet::Nodelet {
...
void PairsIcmImuDriver::onInit() {

  ros::NodeHandle nh = nodelet::Nodelet::getMTPrivateNodeHandle();

  ros::Time::waitForValid();

  timer_imu_     = nh.createTimer(5000, &PairsIcmImuDriver::timerImu, this);
...
void PairsIcmImuDriver::timerImu([[maybe_unused]] const ros::TimerEvent& te) {
  ...
  if (ret == 8) {  // Check if all 8 values were successfully read
    ...
    try {
      ROS_INFO_ONCE("[PairsIcmImuDriver]: publishing IMU data");
...
PLUGINLIB_EXPORT_CLASS(pairs_icm_imu_driver::PairsIcmImuDriver, nodelet::Nodelet);
